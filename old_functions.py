    ...
    #!/usr/bin/env python3
"""
MCP Server wrapper for TTS engines

This server provides access to two text-to-speech engines:
1. Kokoro TTS (local)
2. OpenAI TTS (cloud-based via API)

Both engines support streaming audio playback while generating.
...
# This prevents interference with the MCP protocol
...
# Try to import termcolor for colored output, but it's optional
try:
    from termcolor import colored
    def colored_log(message, color="white"):
        log(colored(message, color))
except ImportError:
    # If termcolor is not available, just use plain text
    def colored_log(message, color="white"):
        log(message)

# Import Kokoro TTS
try:
    # First try to import directly (assuming the script is named speak.py)
    try:
        from speak import KokoroTTS
        colored_log("Successfully imported KokoroTTS from speak.py", "green")
    except ImportError:
        # If that fails, try to find the script in the current directory
        script_paths = [
            Path("/Users/kristoffervatnehol/Documents/MCP_Servers/MCP_tts_server/speak.py"),
            Path("./speak.py"),
        ]
        ...
        for script_path in script_paths:
            if script_path.exists():
                ...
                sys.path.append(str(script_path.parent.absolute()))
                # Extract the module name (without .py)
                module_name = script_path.stem
                # Import the module
                exec(f"from {module_name} import KokoroTTS")
                colored_log(f"Successfully imported KokoroTTS from {script_path}", "green")
                script_found = True
                break
        
        if not script_found:
            raise ImportError("Could not find the script containing KokoroTTS class")

except ImportError as e:
    colored_log(f"Error importing KokoroTTS: {e}", "red")
    colored_log("Kokoro TTS functionality will not be available.", "yellow")
    KokoroTTS = None

# Try to import dotenv for API key management (optional)
try:
    from dotenv import load_dotenv
    load_dotenv()  # Load variables from .env file
    colored_log("Loaded environment variables from .env file", "green")
except ImportError:
    # Dotenv is optional, so continue if not available
    pass

# Check if OpenAI API key is available
OPENAI_API_KEY = os.getenv("OPENAI_API_KEY")
if not OPENAI_API_KEY:
    colored_log("Warning: No OpenAI API key found in environment variables", "yellow")
    colored_log("OpenAI TTS functionality will be disabled", "yellow")
    colored_log("Set the OPENAI_API_KEY environment variable to enable it", "yellow")
    OPENAI_AVAILABLE = False
else:
    # Only try to import OpenAI if the API key is available
    try:
        ...
        colored_log("OpenAI TTS functionality will not be available.", "yellow")
        colored_log("To enable it, install: pip install openai", "yellow")
        OPENAI_AVAILABLE = False
...
class OpenAITTS:
    def __init__(self, api_key=None):
        ...
        if not OPENAI_AVAILABLE:
            raise ImportError("OpenAI package is not installed or API key is not available")
            
        # Get API key from parameter, environment variable, or .env file
        self.api_key = api_key or os.getenv("OPENAI_API_KEY")
        
        if not self.api_key:
            colored_log("Warning: No OpenAI API key provided.", "yellow")
            colored_log("Please set the OPENAI_API_KEY environment variable or pass the key to the constructor.", "yellow")
        ...
        try:
            ...
            colored_log("OpenAI TTS client initialized", "green")
            
            # Define valid voices and formats
            ...
            self.default_format = "mp3"  # Default for downloading
            self.streaming_format = "wav"  # Better for streaming
            self.default_model = "gpt-4o-mini-tts"
            
            # Initialize audio player for streaming
            self.audio_player = LocalAudioPlayer()
        except Exception as e:
            colored_log(f"Error initializing OpenAI client: {str(e)}", "red")
            raise
    
    async def stream_and_play(self, text, voice=None, instructions=None):
        """Generate speech and stream it directly to audio output using OpenAI's streaming API.
        Similar to Kokoro's stream_and_save but without exposing file paths.
        
        Args:
            text: Text to convert to speech
            voice: Voice to use
            instructions: Optional instructions to customize the voice
            
        Returns:
            Success message
        """
        try:
            # First try to use the direct streaming API (preferred method)
            try:
                # Validate parameters
                voice = voice if voice in self.valid_voices else self.default_voice
                
                colored_log(f"Generating and streaming speech: {text[:100]}{'...' if len(text) > 100 else ''}", "cyan")
                colored_log(f"Voice: {voice}", "cyan")
                if instructions:
                    colored_log(f"Instructions: {instructions}", "cyan")
                
                # Track generation time
                start_time = time.time()
                
                # Use streaming API with async client
                params = {
                    "model": self.default_model,
                    "voice": voice,
                    "input": text,
                    "response_format": self.streaming_format,  # wav is recommended for streaming
                }
                ...
                async with self.async_client.audio.speech.with_streaming_response.create(**params) as response:
                    # This will stream the audio directly to speakers
                    await self.audio_player.play(response)
                
                elapsed = time.time() - start_time
                colored_log(f"Speech streaming completed in {elapsed:.2f}s", "green")
                
                return True
                
            except Exception as streaming_error:
                # If streaming fails, log the error and fall back to the non-streaming method
                colored_log(f"Streaming method failed: {str(streaming_error)}", "yellow")
                colored_log("Falling back to non-streaming method...", "yellow")
                raise  # Re-raise to trigger fallback
        
        except Exception:
            # Fall back to generate and save approach
            try:
                # Validate and set defaults
                voice = voice if voice in self.valid_voices else self.default_voice
                format = self.default_format
                
                colored_log(f"Generating speech for text: {text[:100]}{'...' if len(text) > 100 else ''}", "cyan")
                colored_log(f"Voice: {voice}, Format: {format}", "cyan")
                if instructions:
                    colored_log(f"Instructions: {instructions}", "cyan")
                ...
                temp_file = os.path.join(temp_dir, f"openai_tts_{int(datetime.now().timestamp())}.{format}")
                
                # Save the audio to temporary file
                await response.astream_to_file(temp_file)
                
                elapsed = time.time() - start_time
                colored_log(f"Speech generation completed in {elapsed:.2f}s", "green")
                ...
                try:
                    if os.path.exists(temp_file):
                        os.unlink(temp_file)
                        colored_log("Temporary file cleaned up", "green")
                except Exception as e:
                    colored_log(f"Warning: Could not clean up file: {e}", "yellow")
                
                return True
                
            except Exception as e:
                error_message = str(e)
                colored_log(f"Error generating speech: {error_message}", "red")
                
                # Handle common errors with more specific messages
                if "authentication" in error_message.lower():
                    raise ValueError("Authentication error. Please check your API key.")
                elif "rate limit" in error_message.lower():
                    raise ValueError("Rate limit exceeded. Please try again later.")
                elif "quota" in error_message.lower():
                    raise ValueError("Quota exceeded. Please check your OpenAI account.")
                else:
                    raise

def play_audio(file_path, verbose=True):
    """
    Play audio using system player (identical to the function in speak.py).
    ...
    if not file_path or not os.path.exists(file_path):
        if verbose:
            colored_log("No audio file to play", "red")
        return
        
    if verbose:
        colored_log(f"Playing audio from: {file_path}", "cyan")
    ...
    if sys.platform == "darwin":  # macOS
        ...
        if verbose:
            colored_log(f"Unsupported platform: {sys.platform}. Audio saved to {file_path}", "yellow")
        return
    
    if verbose:
        colored_log("Playback complete!", "green")

# Initialize FastMCP server
mcp = FastMCP("tts_server")

# Initialize the TTS engines
kokoro_tts = KokoroTTS() if KokoroTTS else None

# Try to initialize the OpenAI TTS engine but handle failures gracefully
openai_tts = None
if OPENAI_AVAILABLE:
    try:
        openai_tts = OpenAITTS()
        colored_log("OpenAI TTS engine initialized successfully", "green")
    except Exception as e:
        colored_log(f"Failed to initialize OpenAI TTS engine: {str(e)}", "red")
        colored_log("OpenAI TTS functionality will be disabled", "yellow")
        OPENAI_AVAILABLE = False
...
async def tts(
    ...
    For detailed voice information and usage guidelines, check the resource: tts://voices before 
    using the tool the first time. (Allways default to Kokoro unless the user explicitly requests OpenAI)

    """
    # Handle Kokoro TTS engine
    if engine.lower() == "kokoro":
        if not kokoro_tts:
            return "Error: Kokoro TTS is not available"
        # Use existing Kokoro implementation
        try:
            # Restrict speed to the allowed range
            adjusted_speed = max(0.75, min(2, speed))
            if adjusted_speed != speed:
                colored_log(f"Speed adjusted from {speed} to {adjusted_speed} to stay within allowed range (0.75-2)", "yellow")
            
            # Fixed parameters for Kokoro
            voice_kokoro = "af_heart"
            format = "wav"
            no_stream = False
            
            # Use the stream_and_save method from KokoroTTS
            output_file = await kokoro_tts.stream_and_save(
                text=text,
                voice=voice_kokoro,
            ...
            return f"Successfully played Kokoro TTS audio for: \"{text[:50]}{'...' if len(text) > 50 else ''}\""
            
        except Exception as e:
            colored_log(f"Error in Kokoro TTS processing: {str(e)}", "red")
            return f"Error in Kokoro TTS processing: {str(e)}"
    
    # Handle OpenAI TTS engine
    elif engine.lower() == "openai":
        if not OPENAI_AVAILABLE or not openai_tts:
            return "Error: OpenAI TTS is not available. Please ensure the OpenAI package is installed and an API key is configured."
        ...
        try:
            ...
            openai_voice = voice if voice and voice in openai_tts.valid_voices else "ash"
            
            # Use the existing OpenAI stream_and_play method
            ...
            if success:
                return f"Successfully played OpenAI TTS audio for: \"{text[:50]}{'...' if len(text) > 50 else ''}\""
            else:
                return "Failed to generate or play audio"
            
        except Exception as e:
            colored_log(f"Error in OpenAI TTS processing: {str(e)}", "red")
            return f"Error in OpenAI TTS processing: {str(e)}"
    
    # Handle invalid engine selection
    else:
        return f"Error: Unknown engine '{engine}'. Valid options are 'kokoro' or 'openai'."


# Updated resource for voice information
...
## Kokoro TTS (Local)
- Default engine for quick responses
- Supports speed adjustment (0.75-2.0)
- Fixed voice (af_heart)
- Use for: Simple announcements, notifications
...
@mcp.tool()
async def tts_info() -> str:
    """
    Get information about the TTS server.
    
    Returns:
        Information about the available TTS engines and their capabilities.
    """
    info = "# TTS Server Information\n\n"
    
    # Kokoro TTS info
    if kokoro_tts:
        info += "## Kokoro TTS (Local)\n"
        info += "- Status: Available\n"
        info += "- Type: Local TTS engine\n"
        info += "- Features: Fast local processing, speed adjustment (0.75-2.0)\n"
        info += "- Voice: af_heart (fixed)\n\n"
    else:
        info += "## Kokoro TTS (Local)\n"
        info += "- Status: Not available\n\n"
    
    # OpenAI TTS info
    if openai_tts and OPENAI_AVAILABLE:
        info += "## OpenAI TTS (Cloud)\n"
        info += f"- Status: Available\n"
        info += f"- Model: {openai_tts.default_model}\n"
        info += "- Type: Cloud-based TTS engine (requires API key)\n"
        info += "- Features: Natural-sounding speech, voice customization, streaming playback\n"
        info += f"- Available voices: {', '.join(openai_tts.valid_voices)}\n"
        info += f"- Recommended voices: {', '.join(['ash', 'ballad', 'coral', 'sage', 'verse'])}\n\n"
    else:
        info += "## OpenAI TTS (Cloud)\n"
        info += "- Status: Not available\n"
        if not OPENAI_API_KEY:
            info += "- Reason: No API key found. Set the OPENAI_API_KEY environment variable.\n\n"
    
    return info

async def tts_help() -> str:
    """
    Get help about using the TTS server.
    
    Returns:
        Help information and usage examples.
    """
    help_text = "# TTS Server Help\n\n"
    help_text += "## Available Tools\n\n"
    
    help_text += "### `tts(text, engine='kokoro', speed=1.0, voice='', instructions='')`\n"
    help_text += "- Convert text to speech using the preferred engine\n"
    help_text += "- `text`: Text to convert to speech\n"
    help_text += "- `engine`: TTS engine to use (\"kokoro\" or \"openai\")\n"
    help_text += "- `speed`: Speech speed between 0.75-2.0 (applies to Kokoro only)\n"
    help_text += "- `voice`: Voice to use (applies to OpenAI only)\n"
    help_text += "- `instructions`: Instructions to customize the voice (applies to OpenAI only)\n\n"
    
    help_text += "### `tts_examples(category='general')`\n"
    help_text += "- Get examples of effective voice instructions for OpenAI TTS\n"
    help_text += "- `category`: Type of examples to retrieve (\"general\", \"accents\", \"characters\", \"emotions\", \"narration\")\n\n"
    
    help_text += "### `tts_info()`\n"
    help_text += "- Get information about the TTS server\n\n"
    
    help_text += "### `tts_help()`\n"
    help_text += "- Get help about the TTS server (this command)\n\n"
    
    help_text += "### `tts_status()`\n"
    help_text += "- Get the current status of the TTS server\n\n"
    
    help_text += "### `tts_stop()`\n"
    help_text += "- Stop the TTS server (not implemented, for future use)\n\n"
    
    help_text += "### `tts_restart()`\n"
    help_text += "- Restart the TTS server (not implemented, for future use)\n\n"
    
    help_text += "## Resource\n\n"
    help_text += "### `tts://voices`\n"
    help_text += "- Information about available TTS voices\n\n"
    
    help_text += "## Examples\n\n"
    help_text += "```\n# Basic usage with Kokoro TTS\ntts(\"Hello, how are you today?\")\n\n"
    help_text += "# Using OpenAI TTS with a specific voice\ntts(\"Hello, how are you today?\", engine=\"openai\", voice=\"ash\")\n\n"
    help_text += "# Using OpenAI TTS with voice customization\ntts(\"Hello, how are you today?\", engine=\"openai\", voice=\"coral\", instructions=\"Speak in a cheerful, friendly tone.\")\n```\n\n"
    
    return help_text

async def tts_status() -> str:
    """
    Get the current status of the TTS server.
    
    Returns:
        Status information about the TTS server.
    """
    status = "# TTS Server Status\n\n"
    ...
    # Check OpenAI TTS status
    status += "## OpenAI TTS\n"
    if openai_tts and OPENAI_AVAILABLE:
        status += "- Status: Available\n"
        status += f"- Model: {openai_tts.default_model}\n"
        status += f"- Default voice: {openai_tts.default_voice}\n\n"
    else:
        status += "- Status: Not available\n"
        if not OPENAI_API_KEY:
...
async def tts_stop() -> str:
    """
    Stop the TTS server.
    
    Returns:
        Response indicating this feature is not implemented.
    """
    return "This functionality is not implemented yet. The server can only be stopped by terminating the process (Ctrl+C)."

async def tts_restart() -> str:
    """
    ...
    Returns:
        Response indicating this feature is not implemented.
    """
    return "This functionality is not implemented yet. To restart the server, please stop and then start it again manually."


async def tts_voices() -> str:
    """
    Get information about available voices for OpenAI TTS.
    
    Returns:
        Information about available TTS voices.
    """
    # Simply return the same content as the resource
    return get_voice_info()

if __name__ == "__main__":
    # Initialize and run the server
    colored_log("Starting TTS MCP server with multiple engines...", "green")
    
    # Print information about available engines
    if kokoro_tts:
        colored_log("✓ Kokoro TTS engine available (local)", "green")
    else:
        colored_log("✗ Kokoro TTS engine not available", "red")
    
    if openai_tts and OPENAI_AVAILABLE:
        colored_log(f"✓ OpenAI TTS engine available (API, model: {openai_tts.default_model})", "green")
        # List the most accurate voices based on research
        core_voices = ["ash", "ballad", "coral", "sage", "verse"]
        colored_log("  Recommended voices: " + ", ".join(core_voices), "cyan")
    else:
        colored_log("✗ OpenAI TTS engine not available", "red")
        if not OPENAI_API_KEY:
            colored_log("  Reason: No API key found. Set the OPENAI_API_KEY environment variable.", "yellow")
    
    # Print information about new resources
    colored_log("✓ Resources and tools available:", "green")
    colored_log("tts_examples() - Get examples of effective voice instructions for OpenAI TTS", "cyan")
    colored_log("tts_voices() - Get information about available voices for OpenAI TTS", "cyan")
    colored_log("tts_info() - Get information about the TTS server", "cyan")
    colored_log("tts_help() - Get help about the TTS server", "cyan")
    colored_log("tts_status() - Get the status of the TTS server", "cyan")
    colored_log("tts_stop() - Stop the TTS server", "cyan")
    colored_log("tts_restart() - Restart the TTS server", "cyan")
    ...
    try:
        ...
        colored_log("Server stopped by user", "yellow")
    except Exception as e:
        colored_log(f"Error running server: {str(e)}", "red")
        sys.exit(1)
...
async def tts(
    ...
    Args:
        ...
        speed: Speed (Kokoro: 0.8-1.2 typical, OpenAI: 0.8-1.2).
